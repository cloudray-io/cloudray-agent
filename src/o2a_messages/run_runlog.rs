...
use crate::generated::pb::a2o::{
    RunlogFailReason, RunlogFailed, RunlogFinished, RunlogOutputFragment, RunlogStarted,
};
...
pub async fn process_run_runlog(message: RunRunlog) -> anyhow::Result<()> {
    ...
    let stdout_handle = tokio::spawn(handle_output(BufReader::new(stdout), runlog_id, false));
    let stderr_handle = tokio::spawn(handle_output(BufReader::new(stderr), runlog_id, true));
